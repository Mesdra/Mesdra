		...
		try {
			...
			for (int i = 0; i < quantDados; i++) {
				statement.executeUpdate(
						"INSERT INTO dados (id_estacao,hash,nome_estacao,data,hora,sensores,created_at,updated_at)"
								+ "VALUES(" + strings[i][2] + ",'" + strings[i][0] + "','" + strings[i][1] + "','"
								+ strings[i][3] + "','" + strings[i][4] + "','" + strings[i][5]
